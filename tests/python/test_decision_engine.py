...
def test_debounce_and_valid_action(mock_spec):
    ...
    d1 = engine.evaluate(
        "pick_yellow", confidence=0.85, entropy=0.4, spec=mock_spec, current_step_index=0
    )
    ...
    d2 = engine.evaluate(
        "pick_yellow", confidence=0.88, entropy=0.3, spec=mock_spec, current_step_index=0
    )
...
def test_uncertain_confidence_and_entropy(mock_spec):
    engine = ProtocolDecisionEngine(
        default_debounce_threshold=1, default_min_confidence=0.7, default_max_entropy=1.4
    )
    engine.reset_step()

    # Low confidence
    d_low_conf = engine.evaluate(
        "pick_yellow", confidence=0.5, entropy=0.4, spec=mock_spec, current_step_index=0
    )
    assert d_low_conf.status == DecisionStatus.STEP_UNCERTAIN

    # High entropy
    d_high_ent = engine.evaluate(
        "pick_yellow", confidence=0.9, entropy=2.1, spec=mock_spec, current_step_index=0
    )
...
def test_wrong_object_detection(mock_spec):
    ...
    d = engine.evaluate(
        "pick_red", confidence=0.85, entropy=0.4, spec=mock_spec, current_step_index=0
    )
...
def test_out_of_sequence_detection(mock_spec):
    ...
    d = engine.evaluate(
        "place_yellow", confidence=0.9, entropy=0.3, spec=mock_spec, current_step_index=0
    )
...
def test_invalid_action(mock_spec):
    ...
    d = engine.evaluate(
        "drink_water", confidence=0.85, entropy=0.4, spec=mock_spec, current_step_index=0
    )
